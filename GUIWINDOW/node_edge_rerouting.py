...
class EdgeRerouting:
    ...
    def getAffectedEdges(self) -> list:
        ...
        :return: List of all edges affected by the rerouting started from this `self.start_socket` :class:`~GUIWINDOW.node_socket.Socket`
        :rtype: ``list``
        """
    ...
    def setAffectedEdgesVisible(self, visibility: bool=True):
        ...
        :param visibility: ``True`` if all the affected :class:`~GUIWINDOW.node_edge.Edge` (s) should be shown or hidden
    ...
    def clearReroutingEdges(self):
        """Remove the helping dashed edges from the :class:`~GUIWINDOW.node_scene.Scene`"""
    ...
    def startRerouting(self, socket: 'Socket'):
        ...
        :param socket: :class:`~GUIWINDOW.node_socket.Socket` where we started the re-routing
